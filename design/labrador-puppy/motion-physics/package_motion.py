...
archive = ROOT.parent / 'puppy-physics-run.zip'
with ZipFile(archive, 'w', compression=ZIP_DEFLATED, compresslevel=6) as output:
    for source in sorted(ROOT.rglob('*')):
        ...
        if not source.is_file() or any(part in ['checks', '__pycache__'] for part in relative.parts) or source.name == '.gitignore':
            continue
        output.write(source, 'motion-physics/' + relative.as_posix())
    # Keep the same-frame comparison functional when the ZIP is unpacked alone.
    for source in sorted((ROOT.parent / 'motion-refined/frames').rglob('*.png')):
        output.write(source, 'motion-refined/frames/' + source.relative_to(ROOT.parent / 'motion-refined/frames').as_posix())
