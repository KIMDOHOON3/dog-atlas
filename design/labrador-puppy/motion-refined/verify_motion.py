...
report['airborne_frames']=[p['frame'] for p in manifest['poses'] if not p['support']]
...
assert report['body_vertical_span_m']>.019 and len(report['airborne_frames'])>=10
assert all(e['loop_max_delta_m']<1e-6 and e['head_relative_tip_range_m'][1]>.008 for e in report['ear_motion'].values())
