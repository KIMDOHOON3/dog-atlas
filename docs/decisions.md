...
## 2026-09-14 — 안내판을 실제 Three.js 오브젝트로 수정

사용자는 HTML 문구를 얹은 안내판을 거절하고 Three.js 모델을 요청했다. 이전 Blender 안내판과 시각적 HTML 오버레이를 제거한다. Three.js에서 판자/기둥/나사/글씨 표면을 함께 생성하고 실제 3D 보드에 대한 Raycaster 클릭으로 장소 목록에 연결한다. 접근성용 의미 링크와 WebGL 대체 이동은 유지하되 장면 위 HTML 장식은 표시하지 않는다.
...
## 2026-09-15 — 장소 페이지를 기존 도감의 시각 체계에 통합

사용자가 다른 페이지와 비교해 장소 UI 강화를 요청했다. 공통 헤더·흰 바탕·갈색 강조와 산세리프 제목, 둥근 카드로 통일한다. 별도 장소 사진이나 지도처럼 보이는 장식은 만들지 않는다. 종류별 빠른 탐색은 기존 필터 URL을 보존하고, 상세의 미제공 항목과 중복 전체 정보는 펼쳐 확인하도록 정리한다. 동반 조건과 길찾기 행동을 우선 배치하며 정보 제공 범위와 실시간 확인의 한계는 유지한다.

## 2026-09-15 — 장소 탐색에 산책 수첩의 표현 적용

사용자는 UI 직관성은 인정했지만 흔하다고 지적했다. 검색 조작을 유지하면서 견종도감의 외출 페이지라는 연결을 강조한다. 자체 목줄/공 선화와 큰 제목, 밑줄 목차, 이름순 번호 목록을 사용하며 기존 3열 둥근 카드는 목록에서 교체한다. 선화는 장소 사진이나 실제 지도를 대신하는 정보가 아니라 장식으로만 취급한다.

## 2026-09-15 — 장식보다 정보 밀도로 장소 목록 정리

사용자는 수첩 방식의 간격과 장식이 AI 디자인처럼 느껴진다고 지적했다. 해당 선화·큰 카피·번호 제작물을 제거하고 검색과 실제 장소가 가까이 이어지게 한다. 주변 검색은 요청할 때 펼치며 위치 동의는 검색 실행에만 유지한다. 새 장식 방향을 더하지 않고 글자 크기·행 간격·정보 정렬로 읽기 밀도를 조절한다.
