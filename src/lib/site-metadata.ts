...
export const DEFAULT_SHARE_IMAGE =
  "/illustrations/card-studies/great-pyrenees-alpine-v1.webp";
...
export function resolveSiteUrl(
  ...
  if (
    ...
    throw new Error(
      "NEXT_PUBLIC_SITE_URL must be an HTTP(S) site origin without credentials, path, query or fragment.",
    );
  }
...
export function createPageMetadata({
  ...
  imageAlt = "피레네산맥 앞에 서 있는 그레이트 피레니즈 수채화",
}: {
  title: string;
  description: string;
  path: string;
