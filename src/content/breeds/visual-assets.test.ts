...
describe("published breed visual assets", () => {
  it("maps all 365 published breeds to existing WebP card and history files", () => {
    expect(breeds).toHaveLength(365);
    ...
    expect(cardPaths.size).toBe(365);
    expect(historyPaths.size).toBe(365);
    expect(cardHashes.size).toBe(365);
    expect(historyHashes.size).toBe(365);
  }, 30000);
});
