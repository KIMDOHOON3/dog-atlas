...
sys.argv=['build_motion.py'];sys.path.insert(0,str(SOURCE));import build as b
...
PARTS=[f'{name}_{part}' for name in LIMBS for part in ['upper','lower','paw']]+['head','tail']
...
def weights(p):
    ...
    result['tail']=smooth(y,1.65,2.06)*smooth(z,1.65,1.95)
...
def pose(phase):
    ...
    for name,pivot,angle in [('head',np.array([0,-.09,.235]),.025*math.sin(phase*4*math.pi+.4)),('tail',np.array([0,.172,.217]),.065*math.sin(phase*2*math.pi))]:
...
def build():
    ...
    for ob in list(bpy.context.scene.objects):
        ...
        b.curves_object(name,p.reshape(count,segments,3)[::4],radii.reshape(count,segments)[::4],mat,surface)
    ...
    manifest={'frames':N,'fps':FPS,'gait':'가벼운 달리기 · 대각선 지지','views':['side','three-quarter'],'coat':'v3의 1/4 밀도 검토용 털','poses':[]}
    for f in range(1,N+2):
        ...
        for name,(_,location,angle) in transforms.items():
            ob=controls[name];ob.location=location;ob.rotation_euler=(angle,0,0);ob.keyframe_insert('location',frame=f);ob.keyframe_insert('rotation_euler',frame=f)
    ...
    render_frames('side',[1,10,20,30,40],True)
