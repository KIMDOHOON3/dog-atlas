...
describe("breed-specific feature cards", () => {
  it("publishes exactly three unique cards for Japanese Spitz and Maltese", () => {
    expect(breedFeatureSets.map((featureSet) => featureSet.slug)).toEqual(["japanese-spitz", "maltese"]);
  ...
  it("keeps feature coverage separate from the catalog used by discovery", () => {
    ...
    expect(breedFeatureSets).toHaveLength(2);
    expect(getBreedFeatures("border-collie")).toBeUndefined();
  });
});
