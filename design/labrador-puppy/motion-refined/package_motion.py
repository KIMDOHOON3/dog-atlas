...
for name, view, duration in [
    ('preview-normal.webp', 'side', 800),
    ('preview-quarter.webp', 'side', 3200),
    ('preview-three-quarter.webp', 'three-quarter', 3200),
    ('preview-three-quarter-normal.webp', 'three-quarter', 800),
]:
    frames = []
    for frame in range(1, 49):
        with Image.open(ROOT / 'frames' / view / f'{frame:03d}.png') as image:
...
archive = ROOT.parent / 'puppy-refined-run.zip'
with ZipFile(archive, 'w', compression=ZIP_DEFLATED, compresslevel=6) as output:
    for source in sorted(ROOT.rglob('*')):
        ...
        if not source.is_file() or any(part in ['checks', '__pycache__'] for part in relative.parts) or source.name == '.gitignore':
            continue
        output.write(source, 'motion-refined/' + relative.as_posix())
    # Keep the same-frame comparison functional when the ZIP is unpacked alone.
    for source in sorted((ROOT.parent / 'motion-run/frames').rglob('*.png')):
        output.write(source, 'motion-run/frames/' + source.relative_to(ROOT.parent / 'motion-run/frames').as_posix())
